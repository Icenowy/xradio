...
void wsm_upper_restart(struct xr819 *hw_priv)
{
	int i = 0;
...
int wsm_reset(struct xr819 *hw_priv, const struct wsm_reset *arg,
		int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm.wsm_cmd_buf;
...
int wsm_scan(struct xr819 *hw_priv, const struct wsm_scan *arg,
		int if_id)
{
	int i;
	int ret;
...
int wsm_stop_scan(struct xr819 *hw_priv, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm.wsm_cmd_buf;
	wsm_cmd_lock(hw_priv);
...
int wsm_set_pm(struct xr819 *hw_priv, const struct wsm_set_pm *arg,
		int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm.wsm_cmd_buf;
...
int wsm_start(struct xr819 *hw_priv, const struct wsm_start *arg,
		int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm.wsm_cmd_buf;
...
int wsm_start_find(struct xr819 *hw_priv, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm.wsm_cmd_buf;
...
int wsm_stop_find(struct xr819 *hw_priv, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm.wsm_cmd_buf;
...
int wsm_map_link(struct xr819 *hw_priv, const struct wsm_map_link *arg,
		int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm.wsm_cmd_buf;
...
/* 3.65 */
int wsm_init_release_buffer_request(struct xr819 *hw_priv, u8 index)
{
...
int wsm_deinit_release_buffer(struct xr819 *hw_priv)
{
...
void wms_send_disassoc_to_self(struct xr819 *hw_priv, struct xradio_vif *priv)
{
	struct sk_buff *skb = NULL;
...
void wsm_lock_tx(struct xr819 *hw_priv)
{
...
int wsm_hwt_tx_confirm(struct xr819 *hw_priv, struct wsm_buf *buf)
{
...
int wsm_handle_rx_confirmation(struct xr819 *hw_priv, int id,
		int interface_link_id, struct wsm_hdr *wsm, struct wsm_buf *wsm_buf,
		struct sk_buff **skb_p) {
	int ret = 0;
	...
	wsm_cmd = hw_priv->wsm.wsm_cmd.cmd & ~WSM_TX_LINK_ID(WSM_TX_LINK_ID_MAX);
	...
	case 0x041C: /* map_link */
		...
		//ret = wsm_generic_confirm(hw_priv, wsm_arg, &wsm_buf);
		//if (ret)
		//	wsm_printk(XRADIO_DBG_ERROR, "wsm_generic_confirm "
...
int wsm_handle_rx_status(struct xr819 *hw_priv, int id, struct wsm_hdr *wsm,
		struct wsm_buf *wsm_buf, struct sk_buff **skb_p) {
	int ret = 0;
	switch (id) {
	case 0x0801:
...
#endif
	if (id & 0x0400) {
		ret = wsm_handle_rx_confirmation(hw_priv, id, interface_link_id, wsm,
				&wsm_buf, skb_p);
	} else if (id & 0x0800) {
		ret = wsm_handle_rx_status(hw_priv, id, wsm, &wsm_buf, skb_p);
	} else {
		ret = -EINVAL;
	}
	out: return ret;
...
int wsm_get_tx(struct xr819 *hw_priv, u8 **data, size_t *tx_len, int *burst,
		int *vif_selected) {
...
// is there a command waiting?
	if (hw_priv->wsm.wsm_cmd.ptr) {
		++count;
		spin_lock(&hw_priv->wsm.wsm_cmd.lock);
		*data = hw_priv->wsm.wsm_cmd.ptr;
		*tx_len = hw_priv->wsm.wsm_cmd.len;
		*burst = 1;
		*vif_selected = -1;
		spin_unlock(&hw_priv->wsm.wsm_cmd.lock);
		return 1;
		...
		for (;;) {
			...
			spin_lock_bh(&priv->ps_state_lock);
			ret = wsm_get_tx_queue_and_mask(priv, &queue, &tx_allowed_mask,
					&more);
			queue_num = queue - hw_priv->tx_queue;

			if (priv->buffered_multicasts && (ret || !more)
			...
			if (ret) {
				...
				if (xradio_queue_get(queue, priv->if_id, tx_allowed_mask, &wsm,
								&tx_info, &txpriv)) {
					spin_unlock(&priv->vif_lock);
					if_pending = 0;
					continue;
				...
				wsm->hdr.id |= cpu_to_le16(WSM_TX_IF_ID(priv->if_id));
				...
				*burst = min(*burst,
						(int) xradio_queue_get_num_queued(priv, queue,
								tx_allowed_mask) + 1);
				else
				*burst = 1;
